...
from centroid_mean import Centroid
...
@torch.no_grad()
def eval_training(epoch=0, centroids=None, tb=True):
    ...
    for (images, labels) in cifar100_test_loader:
        ...
        feature, outputs = net(images)
        feature = torch.nn.functional.normalize(feature, dim=1)

        feature_mean = torch.mean(feature, dim=-1 , keepdim=True)
        
        feature = torch.cat((feature_mean, feature), 1)
        ...
        if centroids:
            Log_feature = torch.log_softmax(feature, 1)
    ...
    print('Test set: Epoch: {}, Average loss: {:.4f}, Accuracy: {:.4f}, CMI Accuracy: {:.4f},Time consumed:{:.2f}s'.format(
        epoch,
...
if __name__ == '__main__':
    ...
    centroids = Centroid(100, net.fc.in_features+1, samples_data, samples_tar, 1, CdecayFactor=0.999)
    net.fc = dummylayer(net.fc) 
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        if args.resume:
            if epoch <= resume_epoch:
                continue
        net.train()
        train(epoch, centroids)
        net.eval()
        centroids.update_epoch(net, cifar100_training_loader)
        acc = eval_training(epoch, centroids)
